...
% Date: Oct, 2020
    ...
    methods (Access = public)
      ...
      function results = dynamic_analysis(this,t_start,t_step,t_end,BDF_order)
          ...
          results{k}.q = [this.q_i;this.q_j];
          results{k}.dq = [this.dq_i;this.dq_j];
          results{k}.ddq = [this.ddq_i;this.ddq_j];
          ...
          time = t_start + (k-1)*t_step;
          this.t = time; % update system time to t_0
          this.BDF_solve_dynamic(1,t_step,results); % BDF method of order 1
          ...
          for k=3:counts
              time = t_start + (k-1)*t_step;
              this.t = time; % update system time to t_0
              this.BDF_solve_dynamic(BDF_order,t_step,results); % BDF method of order 2
      ...
      function check_pos_vel(this)
          ...
          if max(abs(Phi_r*this.dr_i+Phi_p*this.dp_i - this.nu))>eps % need to update when there are more bodies
      ...
      function BDF_solve_dynamic(this,BDF_order,step_size,recent_solution)
          ...
          switch BDF_order
              case 1
                  s1 = recent_solution{end}; % most recent solution
                  beta = 1;
                  a1 = -1;
                  C_r_dot = -a1*s1.dr_i;
                  C_r = -a1*s1.r_i + beta*h*C_r_dot;
                  C_p_dot = -a1*s1.dp_i;
                  C_p = -a1*s1.p_i + beta*h*C_p_dot;
                  
              case 2
                  s1 = recent_solution{end}; % most recent solution
                  s2 = recent_solution{end-1};
                  beta = 2/3;
                  a1 = -4/3;
                  a2 = 1/3;
                  C_r_dot = -a1*state1.rdot -a2*state2.rdot;
                  C_r = -a1*state1.r -a2*state2.r + beta*h*C_r_dot;
                  C_p_dot = -a1*state1.pdot -a2*state2.pdot;
                  C_p = -a1*state1.p -a2*state2.p + beta*h*C_p_dot;
          ...
          for k = 1:itCountMax
              ...
              this.r_i     = Cr + beta0^2*h^2*rddot;
              this.p_i     = Cp + beta0^2*h^2*pddot;
              this.dr_i  = Crdot + beta0*h*rddot;
              this.dp_i  = Cpdot + beta0*h*pddot;
