function [Phi,nu,gamma,Phi_r,Phi_p] = GCon_D(i,qi,dqi,siP_bar,j,qj,dqj,sjQ_bar,ft,dft,ddft,output)
...
% compute gamma
if output == 0 || 3
    gamma = -2*dij'*pa2B(dpj,sjQ_bar)*dpj + 2*dij'*pa2B(dpi,siP_bar)*dpi - 2*(dij_dot)'*dij_dot + ddft;
end
...
if output == 0 || 4
    if j ~=0
        Phi_r = [-2*dij' 2*dij'];
        Phi_p = [-2*dij'*pa2B(pi,siP_bar) 2*dij'*pa2B(pj,sjQ_bar)];
    else
        Phi_r = -2*dij';
        Phi_p = -2*dij'*pa2B(pi,siP_bar);
    end
end
